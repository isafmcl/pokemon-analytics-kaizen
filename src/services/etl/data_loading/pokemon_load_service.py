...
class PokemonLoadService:
    ...
    def load_pokemon(
        ...
        *,
        ...
        # Safety filter: remove any Pokemon with ID > 799 (outside valid range)
        pokemon_df_valid = pokemon_df[
            pd.to_numeric(pokemon_df[id_column], errors="coerce") <= 799
        ].copy()
        if len(pokemon_df_valid) < len(pokemon_df):
            self._logger.warning(
                "Filtrando %d Pokemon com ID > 799",
                len(pokemon_df) - len(pokemon_df_valid),
            )

        pokemon_ids = [int(x) for x in pokemon_df_valid[id_column].tolist()]
        existing_ids = set(self._repository.get_existing_pokemon_ids(pokemon_ids))

        new_rows = pokemon_df_valid[~pokemon_df_valid[id_column].isin(existing_ids)]
        existing_rows = pokemon_df_valid[pokemon_df_valid[id_column].isin(existing_ids)]
